<div class="title main-np"><span>Просмотренные товары</span></div>
<div class="row">
    <?
    $history = $this->visited_prods->getall();
    $prods_ids = '';
    $i=0;
    foreach($history as $v) {
        $prod_id = substr($v->url, 6+strpos($v->url, '/prod-'), strlen($v->url));
        if($prod_id) {
            if($i==0) {
                $prods_ids .= $prod_id;
            } else {
    ...
    if(strlen($prods_ids)) {
        $Prod = new Model_Prod();
        $prods = $Prod->getall(array('where' => 'visible=1 and id in('.$prods_ids.')'));
                    ...
                    <div class="img-wrapper">
                        <a href="/catalog/prod-<?= $prod->id ?>">
                            <?if(file_exists('pic/prod/'.$prod->id.'.jpg')) {?>
                                <img src="/thumb?src=pic/prod/<?= $prod->id ?>.jpg&amp;width=665&amp;height=665" alt="<?=$prod->name?>">
                            <?} else {?>
