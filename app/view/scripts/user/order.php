<?
$Order = new Model_Order($this->order->id);
?>
<h1>Заказ №<?= $this->order->id ?></h1>

<p>Дата заказа: <?= date("d.m.Y", $this->order->tstamp) ?> <br>
	Статус: <?switch($this->order->status) {
...
<?	if ($this->order->delivery != 4) {?>
<p>
	<?=$this->order->surname ? $this->order->surname.', ' : ''?>
	<?=$this->order->name ? $this->order->name.'. ' : ''?>
	<?=$this->order->country ? $this->order->country.', ' : ''?>
	<?=$this->order->city ? $this->order->city.', ' : ''?>
	<?=$this->order->addr ? $this->order->addr.'.' : ''?>
</p>
<?	} else {?>
<p>
	<?=$this->order->surname ? $this->order->surname.', ' : ''?>
	<?=$this->order->name ? $this->order->name.'. ' : ''?>
	<?=$this->order->country ? $this->order->country.', ' : ''?>
	<?=$this->order->city ? $this->order->city.'<br>' : ''?>
	Номер отделения: <?=$this->order->sklad ? $this->order->sklad.'<br>' : ''?>
</p>
<?	}?>
...
$esystem = $Esystem->get($this->order->esystem);
?>
<p><?=$esystem->name?></p>

<p style="float: right;">Итого заказ: <?= Func::fmtmoney($Order->cart->amount()) . $this->sett["valuta"] ?></p>
...
<div>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="2">
		...
		<tr>
			...
			<td align="right"><?=Func::fmtmoney($Order->cart->amount_without_discount())?>&nbsp;<?= $this->valuta['name'] ?></td>
		</tr>
		<tr>
			<td align="right">Скидка:</td>
			<td align="right"><?=Func::fmtmoney($Order->cart->amount_without_discount() - $Order->cart->amount())?>&nbsp;<?= $this->valuta['name'] ?></td>
		</tr>
		<tr>
			<td align="right">Итого:</td>
			<td align="right"><b><?=Func::fmtmoney($Order->cart->amount())?>&nbsp;<?= $this->valuta['name'] ?></b></td>
		</tr>
	</table>
