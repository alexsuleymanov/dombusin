...
<div class="row">
	...
	<div class="col-sm-8">
		...
		<table class="table table-detail">
			<tbody>
			<tr>
				<td class="productInfoKey" colspan="2">Другие цвета товара</td>
			</tr>
			<tr>
				<td class="productInfoKey" colspan="2">
					<?$Prod = new Model_Prod();
					...
					<div class="pl-colors">
						...
						foreach($cprods as $cprod) {
							if (file_exists('pic/prod/'.$cprod->id.'.jpg')) {
								?>
								<a href="<?=$this->url->mk('/catalog/prod-'.$cprod->id)?>"><img src="/thumb?src=pic/prod/<?=$cprod->id?>.jpg&amp;width=28&amp;height=28" alt="<?=$cprod->name?>" data-id="<?= $cprod->id ?>"<?if($c>$ccount){?> class='color-hidden'<?}?>></a>
								<?
							}
							if($c++==$ccount) {?>
			...
			<tr>
				<td class="productInfoKey">Упаковка:</td>
