						...
						<div class="box-product">
							...
							<form action="<?=$this->url->gvar("buy=1")?>" method="post" id="prodform_<?=$prod->id?>">
								...
								<div class="col-md-9 col-xs-9 col-nopadding">
									<button class="btn btn-theme m-b-1 form-control" type="button" onmousedown="try { rrApi.addToBasket(<?=$prod->id?>) } catch(e) {}" onclick="ga('send', 'event', 'Buy', 'Click_buy'); buy(<?=$prod->id?>); return false;"><i class="fa fa-shopping-cart"></i> В корзину</button>
								</div>
